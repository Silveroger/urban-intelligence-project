...
export function VideoProcessingHub({
          ...
          if (data.last_result?.events) {
            ...
            if (onNewEventRef.current) {
              data.last_result.events.forEach((evt) => {
                const normType: 'road_defect' | 'waterlogging' | 'traffic' | 'incident' =
                  evt.event_type === 'infrastructure'
                    ? 'road_defect'
                    : evt.event_type === 'pedestrian'
                      ? 'incident'
                      : (evt.event_type as 'road_defect' | 'waterlogging' | 'traffic' | 'incident') || 'road_defect';
  ...
  const handleStartProcessing = async () => {
    ...
    formData.append('enabled_detectors', JSON.stringify({
      ...
      pedestrian: (filters.eventTypes as any).pedestrian ?? true,
    }));
