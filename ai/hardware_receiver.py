...
class SmartBusHardwareReceiver:
    def __init__(
        ...
        self.adapter = BackendIngestAdapter(backend_url) if backend_url else None
    ...
    def _post_telemetry_to_backend(self, gps: Dict):
        ...
        if self.adapter:
            threading.Thread(target=lambda: self.adapter.send_telemetry(payload), daemon=True).start()
    ...
    def _post_observation_to_backend(self, event_payload: Dict):
        ...
        if self.adapter:
            threading.Thread(target=lambda: self.adapter.send_observation(event_payload), daemon=True).start()

    def _post_incident_to_backend(self, incident_payload: Dict):
        if self.on_event_callback:
            self.on_event_callback({"type": "NEW_INCIDENT", "payload": incident_payload})

        if self.adapter:
            threading.Thread(target=lambda: self.adapter.send_incident(incident_payload), daemon=True).start()
